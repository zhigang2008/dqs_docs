...
<script type="text/javascript"> 
        ...
        function completeAction(event, queueID, fileObj, response, data){
        	//转换为json对象
             ...
             $("#fileShowName").html(fileObj.name);
        }
</script>

<title>标准工作程序编辑</title>
</head>
...
<body>
	...
	<s:form id="myForm" action="/office/StandardWork/update.do" method="post">
        ...
        <div id="formdiv" >
         <p>基本信息:</p>
         <!-- 隐藏信息 -->
		<s:hidden label="上传文件" id="documentFile" name="fileName" value="%{model.fileName}"  cssClass="required" required="false" />
		<table id="formtable" class="formtable">
		<%@ include file="form_include.jsp" %>
		<tr>
		<td class="tdlabel">状态: </td>
		<td>
		   <s:select label="状态" key="status"  value="%{model.status}" list="%{baseCode.documentStatus}" listKey="id" listValue="name"  cssClass="required  " required="true" />
		 </td>
		</tr>
		<tr>
		<td class="tdlabel">已上传文件: </td>
		<td>
		    <input id="file_upload" name="file_upload" type="file"  />
		    <div id="fileShowName"><s:property value="%{model.fileName}"/></div>
		 </td>
		</tr>
		</table>
		</div>
