...
<body>
	...
	<form id="queryForm" name="queryForm" action="<c:url value="/office/StandardWork/list.do"/>" method="post" style="display: inline;">
	<div class="queryPanel">
		<fieldset>
			...
			<table>
				<tr>	
					...
					<td>
						<s:select value="%{model.type}" id="type" name="type" list="%{baseCode.standardWorkType}" listKey="id" listValue="typeName" headerKey="" headerValue=""  />
					</td>
				...
				<tr>	
					...
					<td>
						<s:select value="%{model.status}" id="status" name="status" list="%{baseCode.documentStatus}" listKey="id" listValue="name" headerKey="" headerValue=""/>
					</td>
		...
		<div class="handleControl">
             <div class="buttons">
               <button type="submit" class="regular" name="query" onclick="getReferenceForm(this).action='${ctx}/office/StandardWork/list.do'" >
                 <img src="<c:url value="/image/form/query.gif"/>" alt="" /> 查询
               </button>
               <wylb:hasPermission name="StandardWork:create">
               <button type="submit" class="positive" name="additem" onclick="getReferenceForm(this).action='${ctx}/office/StandardWork/create.do'" >
                 <img src="<c:url value="/image/form/add.gif"/>" alt="" /> 新增
               </button> 
               </wylb:hasPermission>
               <wylb:hasPermission name="StandardWork:delete">
               <button type="button" class="negative" name="deleteitem" onclick="batchDelete('${ctx}/office/StandardWork/delete.do','items',document.forms.queryForm)">
	...
	<div class="cl"></div>
	<s:if test="#request.page.result!=null">ssss</s:if>
	<div class="gridTable">
		...
		<table width="100%"  border="0" cellspacing="0" class="gridBody">
		  <thead>
			  <tr>
				...
				<th sortColumn="SUBMITTER_ID" >发布者</th>
		  ...
		  <tbody>
		      <s:iterator value="#request.page.result" var="item" status="itemStatus">
		       <tr class="<s:if test="#itemStatus.odd == true ">odd</s:if><s:else>even</s:else>">
				<td>${page.thisPageFirstElementNumber + itemStatus.index}</td>
				<td><input type="checkbox" name="items" value="id=${item.id}&" /></td>
				<td>${item.standardWorkType.typeName}&nbsp;</td>
				<td>${item.workName}</td>
				<td>${item.version}</td>
				<td>${item.author}&nbsp;</td>
				<td>${item.department}&nbsp;</td>
				<td>${item.submitter.realName}&nbsp;</td>
				<td>${item.submitTimeString}&nbsp;</td>
				<td>${item.documentStatus.name}&nbsp;</td>
				<td><wylb:fileSize>${item.fileSize}</wylb:fileSize>&nbsp;</td>
				<td>
					<wylb:hasPermission name="StandardWork:download">
					<s:url id="downloadUrl" value="/support/download.do">
                       <s:param name="fileName" value="%{workName}" />
                       <s:param name="filePath" value="%{fileName}" />
                    </s:url>
                    <s:a href="%{downloadUrl}" title="下载" method="post">
                      <img src="<s:url value="/image/form/download.gif"/>" border="none" alt="下载"/>
                    </s:a>
					</wylb:hasPermission>
					<wylb:hasPermission name="StandardWork:edit">
					<a href="${ctx}/office/StandardWork/edit.do?id=${item.id}&" title="修改" > <img src="<c:url value='/image/form/edit.gif'/>" alt="修改"  /></a>
					</wylb:hasPermission>
				</td>
			  </tr>
		      </s:iterator>
