        ...
        $(document).ready(function() {
			...
			*/
            ...
            $("#typeId").change(function() {
            	$.getJSON("<s:url value="/office/ReportInformationType/jsonGetById.do"/>?id="+$(this).val(), function(json){
            		  if(json.dataMap.template!=null){
            		    $.fck.setHTML("InformationContent",json.dataMap.template.typeTemplate);
            		  }
            		});
            }
            );
...
<body>
	...
	<s:form id="myForm" action="/office/ReportInformation/update.do" method="post">
		<div id="controlbuttons" class="buttons">
               ...
               <button type="button" class="regular" name="backlist" onclick="window.location='${ctx}/office/ReportInformation/list.do'" >
		...
		<td class="tdlabel">
		  ...
		  <td>
		    <s:select value="%{model.status}" id="status" name="status" list="%{baseCode.reportInformationStatus}" listKey="id" listValue="statusName" />
	      </td>
	    </tr>
	    <tr>	
		<td class="tdlabel">
			是否公开<span class="required">*</span>:
		</td>	
		  <td>
		    <s:radio list='#{true:"公开",false:"不公开"}'  key="isPublic" value="%{model.isPublic}" cssClass="required " required="true"></s:radio>
	      </td>
	    </tr>
